...
AddFileToS3 ...
*/
func (s *s3Model) AddFileToS3(imageFileHeader *multipart.FileHeader, filePath string) (models.Info, error) {
...
func (s *s3Model) AddLocalFileToS3(file io.Reader, filePath string) (models.Info, error) {
	var result models.Info
...
上傳檔案
*/
func UploadFileToS3(setting models.Setting, fileHeader *multipart.FileHeader, filePath string) (models.Info, error) {
	var result models.Info
	...
	buffer := make([]byte, fileHeader.Size) // read file content to buffer
	file.Read(buffer)
	contentType := http.DetectContentType(buffer)
	fileBytes := bytes.NewReader(buffer)
	...
	sess, err := session.NewSession(&aws.Config{
		...
		Credentials: credentials.NewStaticCredentials(setting.AccessKeyID, setting.SecretKey, "")},
	)
	...
	if err != nil {
		log.Printf("Unable to upload %q to %q, %v", filePath, setting.Bucket, err)
		return result, err
	}
